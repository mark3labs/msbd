...
- **A new section** = a real page: add a `pageX` handler in `handlers.go` that calls `h.render(w, r, views.SectionX, "Title", "", views.XPage(...))`, register `GET /dashboard/x`, add a `Section` const + a `navItem` in `views/layout.templ`. Users expect to bookmark it.
- **A new in-page update** = an SSE fragment: add the handler under `/dashboard/api/...`, have it `PatchElementTempl` a fragment whose root `id` matches the element already on the page.
