...
- **A new section** = a real page: add a `pageX` handler in `handlers.go` that calls `h.render(w, r, views.SectionX, "Title", "", views.XPage(...))`, register `GET /dashboard/x` **with the right guard** (`h.guardPage` normally, `h.guardAdminPage` for anything under Settings), add a `Section` const + a `navItem` in `views/layout.templ`. Users expect to bookmark it.
- **A new in-page update** = an SSE fragment: add the handler under `/dashboard/api/...` behind `h.guardAPI` (read) or `h.guardWrite` (mutation), and have it `PatchElementTempl` a fragment whose root `id` matches the element already on the page.
