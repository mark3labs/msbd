...
2. **Choose a filename**: short, lowercase, hyphen-separated, descriptive (e.g. `smoke-test.md`)
...
4. **Decide on arguments**:
   ...
   - One required value (issue number, sandbox id, file path) → use `$1`
...
5. **Draft the body**:
   ...
   - Be specific: name commands, flags, and file paths where relevant. For this repo that means real things like `task test:race`, `internal/core/service.go`, `openapi.yaml`
...
7. **Verify substitution** by mentally replacing `$1`/`$@` with a sample value and confirming every reference resolves — and that the prompt's *own* example snippets don't accidentally bump the required-arg count (wrap illustrative `$N` examples in triple-backtick fences, not 4-space indentation, so the arg counter ignores them)
8. **Confirm** by showing the final file content and the slash command that activates it (e.g. `/smoke-test 42`)
