...
4. **Craft the commit message** following Conventional Commits:
   ...
   - Scope: optional, the subsystem affected — for this repo: `api`, `core`, `cmd`, `openapi`, `nix`, `docker`, `ci`, `docs`
...
- If the change touches `internal/api/dto.go`, check whether `openapi.yaml` moved in lockstep — call it out in the body if the wire contract changed (DTOs are the contract; see AGENTS.md)
